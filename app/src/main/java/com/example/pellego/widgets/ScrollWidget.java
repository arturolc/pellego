...
import com.example.pellego.app.PDFPlugin;
import com.example.pellego.app.Plugin;
import com.example.pellego.app.Reflow;
...
public class ScrollWidget extends RecyclerView implements ZLViewWidget {
    ...
    public class ScrollAdapter extends Adapter<ScrollAdapter.PageHolder> {
        ...
        public class PageHolder extends ViewHolder {
        ...
        lm = new LinearLayoutManager(fb.getContext()) {
                ...
                public void run() {
                    if (idley >= 0) {
                        ...
                        if (next < adapter.pages.size()) {
                            ViewHolder h = findViewHolderForAdapterPosition(next);
                            if (h != null)
                        ...
                        int prev = page - 1;
                        if (prev >= 0) {
                            ViewHolder h = findViewHolderForAdapterPosition(prev);
                            if (h != null)
                                h.itemView.draw(new Canvas());
                        }
                    }
            ...
            public void smoothScrollToPosition(RecyclerView recyclerView, State state, int position) {
                ...
                if (Build.VERSION.SDK_INT >= 21 && pm.isPowerSaveMode()) {
                    ...
                    SmoothScroller smoothScroller = new TopAlwaysSmoothScroller(recyclerView.getContext());
